...
trait SearchQueries[T] { this: BaseQueries[T] =>
  protected def onCountAll(filters: Seq[Filter] = Nil) = {
    new Count(sql = s"select count(*) as c from `$tableName`${filterClause(filters, fields).getOrElse("")}", values = filters.flatMap(_.v))
  }

  private[this] def searchClause(q: String) = searchColumns.map(c => s"""lower(`$c`) like ?""").mkString(" or ")
