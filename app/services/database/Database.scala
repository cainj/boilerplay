...
object Database extends Instrumented with Logging {
  ...
  def execute(statement: Statement, conn: Option[Connection] = None): Future[Int] = {
    ...
    logger.debug(s"Executing statement [$name] with SQL [${statement.sql}] with values [${statement.values.mkString(", ")}].")
    ...
    ret.failed.foreach { x =>
      logger.error(s"Error [${x.getClass.getSimpleName}] encountered while executing statement [$name] with SQL [${statement.sql}].")
      logger.error(x)
    }
    ret
  }
  ...
  def query[A](query: RawQuery[A], conn: Option[Connection] = None): Future[A] = {
    ...
    logger.debug(s"Executing query [$name] with SQL [${query.sql}] with values [${query.values.mkString(", ")}].")
    ...
    ret.failed.foreach { x =>
      logger.error(s"Error [${x.getClass.getSimpleName}] encountered while executing query [$name] with SQL [${query.sql}].")
      logger.error(x)
    }
    ret
  }
  ...
  def raw(name: String, sql: String, conn: Option[Connection] = None): Future[QueryResult] = {
    logger.debug(s"Executing raw query [$name] with SQL [$sql].")
    val ret = metrics.timer(s"raw.$name").timeFuture {
      conn.getOrElse(pool).sendQuery(prependComment(name, sql))
    }
    ret.failed.foreach { x =>
      logger.error(s"Error [${x.getClass.getSimpleName}] encountered while executing raw query [$name] with SQL [$sql].")
      logger.error(x)
    }
    ret
  }
